...
def compare_experiments(experiments, metric_name, title="Experiment Comparison"):
    """
    Compare experiments based on a specific metric.

    Args:
        experiments (list): List of ExperimentTracker objects.
...
def interactive_plot_metric_trends(metrics, labels, title="Metric Trends"):
    ...
    # Add traces (lines) for each experiment
    for metric, label in zip(metrics, labels):
...
# --- 3. Residual Plot for Regression ---
...
# --- 4. Real-Time Monitoring with Dash ---
